...
class ReActProtocol:
    ...
    def parse(
        ...
        if self.finish['begin'] in message:
            ...
            return thought, action_executor.finish_action.__name__, {"response":final_answer}

        action_regex = f"{self.action['begin']}(.*?)\n"
        args_regex = f"{self.action_input['begin']}(.*)"
        action_match = re.findall(action_regex, message)
        if not action_match:
            return thought, action_executor.no_action.__name__, ''
        ...
        return thought.strip(), action.strip(), action_input.strip().strip('"')
    ...
    def format_response(self, action_return: ActionReturn) -> dict:
        ...
        return dict(
            role='system',
            content=self.response['begin'] + action_return + self.response['end'])
...
class ReAct(BaseAgent):
    ...
    def chat(self, message: Union[str, dict, List[dict]],
        ...
        for turn in range(self.max_turn):
            ...
            response = self._llm.chat(prompt, **kwargs)['content'].replace('```','').strip()
            ...
            if action_return.type == self._action_executor.finish_action.name:
                ...
                break
            inner_history.append(self._protocol.format_response(action_return.result))
        else:
