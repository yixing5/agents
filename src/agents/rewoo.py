...
3. 工具参数可以是正常输入json文本, 或是 #E[依赖的索引], 或是两者都可以。
4. 工具名称必须从一下工具中选择：
{tool_description}
...
class ReWOOProtocol:
    ...
    def parse_worker(
        ...
        action_units = re.findall(r'#E\[\d+\] = (.*)', message)
...
class ReWOO(BaseAgent):
    ...
    def chat(self, message: Union[str, dict, List[dict]],
        ...
        while turn_id < self.max_turn:
            ...
            response = self._llm.chat(planner_prompt, **kwargs)["content"]
        ...
        for action_id in range(len(actions)):
            ...
            for prev_ptr in prev_ptrs:
                ptr_num = int(prev_ptr.strip('#E').strip('[').strip(']')) - 1  # start from 0
                actions_input[action_id] = actions_input[action_id].replace(
                    prev_ptr, action_responses[ptr_num])
            action_return: ActionReturn = self._action_executor(
                actions[action_id], actions_input[action_id])
            action_responses.append(action_return.result.replace('"',"'"))
        ...
        final_response = self._llm.chat(solver_prompt, **kwargs)["content"]
