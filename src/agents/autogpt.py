...
from src.schema import ActionReturn, ActionStatusCode, AgentReturn
...
class AutoGPTProtocol:
    ...
    def prase_json_from_response(self,rsp: str):
        pattern = r"```json(.*?)```"
        rsp_json = None
        try:
    ...
    def parse(self, response: str,
        ...
        try:
            if response.startswith('```') and response.endswith('```'):
                ...
                response = self.prase_json_from_response(response)
            try:
                if 'command' not in response:
                    return action_executor.no_action.__name__, "Missing 'command' object in JSON"
                if not isinstance(response, dict):
                    return action_executor.no_action.__name__, f'The previous message sent was not a dictionary {response}'
                command = response['command']
                if not isinstance(command, dict):
                    return action_executor.no_action.__name__, "'command' object is not a dictionary"
                if 'name' not in command:
                    return action_executor.no_action.__name__, "Missing 'name' field in 'command' object"
                command_name = command['name']
                # Use an empty dictionary if 'args' field is not present in 'command' object
                arguments = command.get('args', {})
                if not command_name.strip():
                    return action_executor.finish_action.__name__, {"response":response["thoughts"]["speak"]} 
                return command_name, arguments
            except Exception as e:
                return action_executor.no_action.__name__, repr(e)
        except SyntaxError as e:
            return action_executor.no_action.__name__, f'Your response could not be parsed: {repr(e)} \nRemember to only respond using the specified format above!'
    ...
    def format_response(self, action_return) -> dict:
        ...
        if action_return.state == ActionStatusCode.SUCCESS:
            response = f'Command {action_return.name} returned: {action_return.result}'
        else:
...
class AutoGPT(BaseAgent):
    ...
    def chat(self, goal: str, **kwargs) -> AgentReturn:
        ...
        for _ in range(self.max_turn):
            ...
            response = self._llm.chat(prompt, **kwargs)['content']
            ...
            if action_return.type == self._action_executor.finish_action.name:
                agent_return.response = action_return.result
                return agent_return
